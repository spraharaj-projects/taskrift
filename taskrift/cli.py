"""This module provides the taskrift CLI."""
...
from taskrift import ERRORS, __app_name__, __version__, config, database, taskrift
...
def get_todoer() -> taskrift.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please run "taskrift init"', fg=typer.colors.RED
        )
        raise typer.Exit(1)
    if db_path.exists():
        return taskrift.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "taskrift init"', fg=typer.colors.RED
        )
        raise typer.Exit(1)
